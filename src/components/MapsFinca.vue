<template>
  <div class="cont w-full flex justify-between flex-wrap items-center gap-2.5 shadow rounded-lg p-4">
    ...
    <div class="card flex flex-col h-140 w-full shadow-none rounded-md sm:card-side max-w-sm sm:max-w-full">
      ...
      <div class="card-body w-[30%] p-0 pl-4 flex justify-start">
        <div class="shadow-sm rounded-md p-4">
          ...
          <div class="card-actions mt-5">
            <RouterLink to="dashboard-plasticos" class="btn bg-[#545386] px-2">Ver historial de mantenimiento</RouterLink>
          </div>
        </div>
      </div>
    </div>
...
function getColorForCategory(category) {
  if (!categoryColors[category]) {
    const colores = ['red', 'red', 'aqua', 'violet', 'black', 'orange', 'lime', 'purple', 'green', 'magenta', 'yellow', 'aquamarine', 'aqua', 'blue'];
...
function irDetalle(id, categoria) {
  console.log(`Ver detalle: ${id}, ${categoria}`);
