        ...
        return `
          <div class="datamap-hoverover" style="background: white; border: 1px solid #ccc; border-radius: 8px; padding: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); min-width: 200px;">
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid #eee;">
              ...
              <span style="font-weight: bold; color: #333;">${geo.properties.name}</span>
            </div>
...
async function loadColombiaTopojson() {
  try {
    ...
    const response = await fetch('/src/assets/colombia.topojson') // Ajusta la ruta según tu estructura
...
function createCustomColombiaMap(container, topojsonData) {
  ...
  const features = window.topojson.feature(topojsonData, topojsonData.objects[Object.keys(topojsonData.objects)[0]])
  ...
  svg.selectAll('path')
    ...
    .style('fill', function(d) {
      ...
      const deptData = colombiaDataSet[d.properties.DPTO] || colombiaDataSet[d.properties.NAME_1] || colombiaDataSet[d.id]
      if (deptData) {
        switch(deptData.fillKey) {
          case 'PRIMARY_FARM': return '#22c55e'
          case 'SECONDARY_FARM': return '#eab308'
  ...
  function showTooltip(d, event) {
    const deptData = colombiaDataSet[d.properties.DPTO] || colombiaDataSet[d.properties.NAME_1] || colombiaDataSet[d.id]
    const name = d.properties.NOMBRE || d.properties.NAME_1 || d.properties.name || 'Región'

    let content = `<div style="background: white; border: 1px solid #ccc; border-radius: 8px; padding: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); min-width: 220px;">
      <div style="font-weight: bold; color: #333; margin-bottom: 8px;">${name}</div>`
...
:global(.datamaps-subunit.COL:hover) {
