<template>
  <header class="fixed z-2 flex justify-between items-center py-5 px-10 min-h-16 w-full bg-white shadow-sm border-b-3 border-[#D8D2C4]">
    <div class="flex items-center gap-4">
      <a @click="enviar" class=" flex items-center text-[#545386] decoration-none mr-4 p-1.5 rounded-sm duration-75 cursor-pointer">
...
async function cargarDatos() {
  try {
    ...
    const feature = geojsonData.value.features.find(f =>
      f.properties.id == id && f.properties.categoria == categoria
    );
    ...
    if (!feature) {
      document.getElementById('contenido').innerHTML = `No se encontró el bloque con ID ${id} y categoría ${categoria}`;
      return;
    }
