...
class HSOverlay extends HSBasePlugin<{}> implements IOverlay {
  ...
  constructor(el: HTMLElement, options?: IOverlayOptions, events?: {}) {
    ...
    this.toggleButtons = Array.from(document.querySelectorAll(`[data-overlay="#${this.el.id}"]`))
  ...
  private elementClick() {
    const payloadFn = () => {
      ...
      dispatch('toggleClicked.overlay', this.el, payload)
    }
  ...
  private buildBackdrop() {
    ...
    const overlayZIndex = parseInt(window.getComputedStyle(this.el).getPropertyValue('z-index'))
    const backdropId = this.el.getAttribute('data-overlay-backdrop-container') || false
    ...
    if (backdropId) {
      this.backdrop = document.querySelector(backdropId).cloneNode(true) as HTMLElement
    ...
    if (closeOnBackdrop) {
      ...
      this.backdrop.addEventListener('click', this.onBackdropClickListener, true)
    }

    this.backdrop.setAttribute('data-overlay-backdrop-template', '')
  ...
  private destroyBackdrop() {
    const backdrop: HTMLElement = document.querySelector(`#${this.el.id}-backdrop`)
  ...
  public close(forceClose = false, cb: Function | null = null) {
    HSOverlay.openedItemsQty = HSOverlay.openedItemsQty <= 0 ? 0 : HSOverlay.openedItemsQty - 1
