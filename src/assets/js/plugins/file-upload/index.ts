...
class HSFileUpload extends HSBasePlugin<IFileUploadOptions> implements IFileUpload {
  ...
  constructor(el: HTMLElement, options?: IFileUploadOptions, events?: {}) {
    ...
    const data = this.el.getAttribute('data-file-upload')
    ...
    this.concatOptions = {
      clickable: this.el.querySelector('[data-file-upload-trigger]') as HTMLElement,
      previewsContainer: this.el.querySelector('[data-file-upload-previews]') as HTMLElement,
  ...
  private initDropzone() {
    const clear = this.el.querySelector('[data-file-upload-clear]') as HTMLButtonElement
  ...
  private onAddFile(file: DropzoneFile) {
    ...
    const reloadButton = file.previewElement.querySelector('[data-file-upload-reload]')
  ...
  private previewAccepted(file: DropzoneFile) {
    ...
    const fileName = previewElement.querySelector('[data-file-upload-file-name]')
    const fileExt = previewElement.querySelector('[data-file-upload-file-ext]')
    const fileSize = previewElement.querySelector('[data-file-upload-file-size]')
    const fileIcon = previewElement.querySelector('[data-file-upload-file-icon]') as HTMLElement
    const trigger = this.el.querySelector('[data-file-upload-trigger]') as HTMLElement
    const preview = previewElement.querySelector('[data-dz-thumbnail]') as HTMLElement
    const remove = previewElement.querySelector('[data-file-upload-remove]') as HTMLButtonElement
  ...
  private onUploadProgress(file: DropzoneFile, progress: number) {
    ...
    const progressBar = previewElement.querySelector('[data-file-upload-progress-bar]')
    ...
    const progressBarValue = previewElement.querySelector(
      '[data-file-upload-progress-bar-value]',
    ) as HTMLElement

    const currentProgress = Math.floor(progress)

    if (progressBar) progressBar.setAttribute('aria-valuenow', `${currentProgress}`)
  ...
  static autoInit() {
    ...
    if (window.$hsFileUploadCollection)
      window.$hsFileUploadCollection = window.$hsFileUploadCollection.filter(({ element }) =>
        document.contains(element.el),
      )

    document
      .querySelectorAll('[data-file-upload]:not(.--prevent-on-load-init)')
      .forEach((el: HTMLElement) => {
        if (!window.$hsFileUploadCollection.find((elC) => (elC?.element?.el as HTMLElement) === el))
          new HSFileUpload(el)
      })
  }
}
...
window.addEventListener('load', () => {
  if (document.querySelectorAll('[data-file-upload]:not(.--prevent-on-load-init)').length) {
